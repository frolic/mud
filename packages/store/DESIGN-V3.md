...
8. **Composite types for keys/values — leading candidate: schema-shaped user types** (direction, not decision; replaced the removed per-type `keySchema` draft and an interim `structs`-section sketch). `userTypes` becomes a discriminated union — **import vs declare**: `{ type, filePath }` imports an existing UDVT (yours or a package's — e.g. `ResourceId` from `@latticexyz/store`, cross-package types like DUST's `EntityId`, or sub-byte/custom packings codegen can't express), while `{ schema }` declares a new composite that codegen authors:

   ```ts
   userTypes: {
     EntityId: { type: "bytes32", filePath: "./src/types/EntityId.sol" },
     Vec3: { schema: { x: "int32", y: "int32", z: "int32" } },
   },
   tables: {
   ...
   For `schema`-shaped types, codegen emits everything: `type Vec3 is uint96` (total component width, ≤32 bytes), component accessors, functional updaters (`withX` — UDVTs are immutable values, no in-place member writes), constructor, and `encode`/`decode`. **Packing order matches `FieldLayout` order, so the codec is a bytes-level identity**: the wrapped word IS the component bytes as they sit in static data — whole-value `load()` is one range read + cast, `save()` is one coalesced splice (one storage op, one event); shift/mask costs apply only to individual component access. As a key, the type expands to N columns named `position_x` etc. (instance naming free via field names — the ambiguity that killed positional `'Vec3[0]'` references); as a value, N tight-packed component fields with per-component offchain columns (partially answering [#1461](https://github.com/latticexyz/mud/issues/1461)). Storage is byte-identical to the hand-packed-primitive approach — MUD's tight-packing lives in `FieldLayout`, not in ABI types. Ownership is split to avoid authoring-order problems: the **user owns a tiny type-definition file** (`src/types/Vec3.sol`: the UDVT declaration + `using Vec3Codec for Vec3 global` + any of their own `using` directives — scaffolded by codegen if missing, written once, never overwritten), while **codegen owns the generated codec library** (`codegen/types/Vec3Codec.sol`: accessors, functional updaters, `encode`/`decode`). The two files import each other — the compile-time cycle is verified fine; the authoring cycle is eliminated by scaffold-on-missing. Because the user owns the defining file, methods and custom operators (`using { add as + }`, UDVT-only) attach with plain `using` lines — no config hook needed; the same scaffold pattern could give enums attachable methods. This subsumes hand-written packed types: DUST's `Vec3.sol` becomes a config line plus a methods file, with stack residency included. Edges: components static-only and primitives-only in v1, total width ≤32 bytes, components count toward the 28-field cap, and no nesting.
