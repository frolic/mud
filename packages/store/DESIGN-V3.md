...
6. **Make the hook tax opt-in.** All four `StoreCore` write paths load `StoreHooks._get(tableId)` before doing anything — a ~100 gas (warm; 2100 cold per table per tx) storage read on **every write**, paid identically by tables with zero hooks. Measured: 4 call sites; DUST registers **zero** store hooks (its programs are app-level dispatch), so its entire write volume pays pure deadweight. Hooks are not in the event EIP, so this is free to rethink: per-table `hooks` flag at registration (default off), generated code routes hook-less tables through a fast-path kernel that skips the lookup. Module-backed tables (KeysWithValue-style reverse mappings, ERC20/721 modules) opt in and keep dynamic hooks. Trade-off: a hook-less table can't gain hooks without re-registration. Hook interface redesign rides along: kernel-aligned 4 ops × before/after, and let `onBefore` return a memory blob that `onAfter` receives ([#3636](https://github.com/latticexyz/mud/issues/3636) — avoids the storage-stack workaround for hooks needing previous + new values).
...
| [#3636](https://github.com/latticexyz/mud/issues/3636) pass data from before to after hooks                                                            | Folded into hook redesign (§2 item 6).                                                                                                                              |
