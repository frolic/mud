...
uint32 wp = Position(player).waypoints().get(2);
Position(player).waypoints().set(2, 7);     // new in v3 (#2019)
...
- **One model everywhere: accessors return handles, operations live on handles.** The record is a handle (`load`/`save`/`destroy` — load/save name the storage I/O and signal that what you hold is a memory snapshot, not a live reference); every field accessor returns a typed field handle (`get`/`set`, plus the dynamic op set for arrays/bytes/strings, plus whatever a user type's lib defines). One sentence describes the whole API, and there is never a second spelling for the same operation.
...
- **Field-handle methods never constrain field names**: `get`/`set`/`length`/`push`/`slice`/extension methods live on framework types (`Int32Field`, `Uint32ArrayField`, …), not in the field-name namespace.
...
**Shared-lib growth (framework PRs, zero regeneration).** Adding indexed `set`/`slice`/`splice` to `Uint32ArrayFieldMethods` lights up every array field in every table ever generated. [#2019](https://github.com/latticexyz/mud/issues/2019) collapses from "extend the generator and regenerate the ecosystem" to "add three functions to one library." Same for `RecordMethods` (e.g. a future `copyTo(Record)`).
...
- **Workload-shaped benchmarks** (DUST profile): a move-loop (warm field reads ×N), an inventory scan (`length` + `getItem` ×N), record load/mutate/save, both compiler pipelines, `forge snapshot` diff against v2 output. Stretch goal: port one real DUST system.
...
| ----------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- | ---------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| [#2019](https://github.com/latticexyz/mud/issues/2019) dynamic-length methods (indexed set/slice/splice)                                                                                  | Addressed: shared dynamic field methods (§2); future additions are library PRs, not codegen changes.                                                                                                                                                                                                                                                                                                                                                                                                                   |
