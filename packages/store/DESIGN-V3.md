...
8. **Composite types for keys/values — leading candidate: config-declared structs** (direction, not decision). The per-user-type `keySchema` draft was removed for mixing concerns; discussion converged on extending the enum precedent: a `structs` config section declares named components of base ABI types, and **codegen emits the struct** alongside enums:

   ```ts
   structs: {
     Vec3: { schema: { x: "int32", y: "int32", z: "int32" }, methods: "./src/Vec3Methods.sol" },
   },
   tables: {
   ...
   Properties: the codec is **automatic** (codegen destructures real members — no user encode/decode, and no wire-format declaration: the store only ever sees components); as a key the struct expands to N columns named `position_x` etc.; as a value, N static fields with per-component offchain columns (partially answering [#1461](https://github.com/latticexyz/mud/issues/1461)); instance naming is free via field names (`from_x` vs `to_x` — the ambiguity that killed positional `'Vec3[0]'` references). The optional `methods` hook lets codegen import a user library and emit `using ... for Vec3 global` itself — verified to compile via circular import (user lib imports the codegen'd struct; codegen imports the lib) — so struct methods are ambient like any other type's; the same hook generalizes to enums. The packed-UDVT tier remains the perf escape hatch: gas-critical math types (DUST's real `Vec3`, a `uint96` UDVT) want one stack slot and custom operators (UDVT-only, 0.8.19+), and stay `userTypes` — if multi-column key expansion is wanted for those, they ship the pure component codec pair (`encode(Vec3) → (int32, int32, int32)` / `decode(...)`); framework owns all `bytes32` padding either way. Packing notes: MUD's tight-packing lives in `FieldLayout`, not in ABI types — three `int32` components occupy the same 12 contiguous static bytes as one `uint96` field, so expansion costs zero storage; and since components are adjacent by construction, the generated struct-field `save()` coalesces into **one** static splice (one storage op, one event), not N field writes. A `packed: true` flag takes it further: codegen emits the UDVT (`type Vec3 is uint96` at total component width, ≤32 bytes), the shift/mask accessors, constructor, and codec — all mechanical from the declaration — giving stack residency and (via the same circular-import hook) custom operators with zero hand-written packing code. That subsumes the hand-written UDVT tier entirely: DUST's `Vec3.sol` becomes a config line plus a methods file; the unpacked memory-struct backing remains for >32-byte shapes or when direct member access is preferred. Edges for the full design: components static-only, count toward the 28-field cap, no nesting in v1, and the codegen-bootstrap ordering for the `methods` import.
