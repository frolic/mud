...
1. **Shrink `IStoreWrite` to the four event-mirroring primitives.** Five of today's nine write methods are convenience wrappers StoreCore already implements _in terms of_ splices — their only job is field-index→byte-range translation (`setStaticField` IS `spliceStaticData(offset)`; `setField` dispatches; `setDynamicField` = splice(0, oldLen); `push` = splice(end, 0); `pop` = splice(end−n, n)). That translation moves into the field method libs: static is a _compile-time-constant_ offset (cheaper than today's runtime `_getStaticDataOffset`), dynamic computes start/deleteCount from the current length. **Caveat for the dynamic three:** they need the field's current length, which StoreCore loads once and threads into the internal splice; a naive call to the _public_ `spliceDynamicData` would re-read `EncodedLengths` (redundant warm SLOAD per push/pop/replace). The shrink should give the public splice a 'push-at-end / delete-to-end' affordance so the caller need not pre-read length. Static removal is pure win; dynamic removal needs that signature tweak to stay gas-neutral. Deletes ~5 methods × (ABI + `StoreCore` impl + `StoreSwitch` mirror + hook touchpoints), and kills the dual indexing scheme (global `fieldIndex` vs relative `dynamicFieldIndex`) — the only addressing left is static byte range and dynamic field N byte range. `StoreSwitch` (~580 lines of per-method mirrors) shrinks to the kernel and is absorbed into the `StoreAccess` dispatch. Hooks follow the kernel: 4 ops × before/after.
...
6. **Store hooks — DECIDED: remove entirely, replace with owner-woven composable effects.** All four `StoreCore` write paths load `StoreHooks._get(tableId)` before doing anything — a ~100 gas (warm; 2100 cold per table per tx) storage read on **every write** (4 measured call sites). A per-table opt-in flag was considered and rejected: it's a one-way door at registration, wrong for long-lived autonomous worlds where unforeseen future functionality is the point. The real question was whether hooks earn their keep at all; they don't.

   **Why remove.** Store hooks are database triggers: a global, runtime, anyone-can-attach dispatch the storage primitive must consult on every op. Evidence they don't pay: the only first-party consumers are `KeysInTable`/`KeysWithValue` — acknowledged-inefficient index modules — and in practice teams found the hook engine too gas-heavy to route through and **already hand-roll bundled index writes in their own write path** (revealed preference). The ERC20 module uses zero store hooks (emits its own events); DUST registers zero.

   **The replacement — reactivity follows write-ownership.** The field-extension model (`using <Trait> for <Field>`) works because the typed handle is the single chokepoint for that field. Writes get the same compile-time, travels-with-the-type, zero-cost-to-non-users extensibility **iff** writes have a single chokepoint — i.e. the table has a single owning namespace that routes all writes through its own setter. Then reactivity is just **composable effect functions the owner's setter calls inline** — the write-side analog of field traits, shipped by the SDK (`KeysInTable.track`, `KeysWithValue.index`, the DUST `ReverseMovablePosition`-alongside-`EntityPosition` pattern). No registry, no per-op read, no external `CALL`; non-reactive tables pay literally zero. The SDK's job shifts from _shipping a dispatch engine_ to _shipping correct, composable, unit-testable write-path effects_ — a docs/typing problem, not a gas problem.

   **What it costs, and the escape hatch.** The trigger's global guarantee ("fires for every writer, even ones I don't control") is replaced by ownership: correctness rests on the table having one writer that routes through the effect-bundled setter. The failure mode is a second writer calling raw `.save()` — so "this table is owned; write through its setter" must be a first-class, enforced convention, not a comment. The one genuine loss — reacting to a table you _don't_ own/write — is reframed as **owner-opt-in, not store-global**: an owner may deliberately publish a reactive interface (emit a domain event / fan out to subscribers for that table only), a scoped capability rather than a default imposed on the primitive. This is strictly smaller surface than today's anyone-hooks-anyone.

   **Mechanically:** delete `IStoreHook`, `Hook.sol`, the `StoreHooks` table, and the before/after machinery — ~100 gas off every write in every world, forever, and [#3636](https://github.com/latticexyz/mud/issues/3636) (before→after data passing) becomes moot.
...
8. **Composite types for keys/values — leading candidate: config-declared, codegen-authored, self-contained** (direction, not decision). Design journey, for the record: per-type `keySchema` (rejected: mixed key/value concerns), positional `'Vec3[0]'` schema references (rejected: instance ambiguity, string-parsing validation), hand-written codec + schema re-declared in config (rejected: duplication), a codegen↔user `methods` hook via circular imports and scaffolded `src/` files (rejected: incomprehensible plumbing), and Solidity-as-source emitting a typed literal for config to import (rejected on principle: **config validation must be fully static — no type-level codegen, no generated inputs, ever**). The resolution is the enum precedent, extended:

   ```ts
   userTypes: {
     EntityId: { type: "bytes32", filePath: "./src/types/EntityId.sol" },        // import: hand-written
     Spawn:    { schema: { x: "int32", y: "int32", active: "bool" } },           // declare: codegen emits the struct
     Vec3:     { schema: { x: "int32", y: "int32", z: "int32" }, packed: true }, // declare: codegen emits the UDVT
   },
   tables: {
   ...
   The shape is declared **once** (config is the single source of truth, statically self-validating). Codegen emits the type into `codegen/types/` exactly like enums and tables — never into `src/`. The generated file is **self-contained**: the type, its codec/accessor library (component accessors, functional `withX` updaters for packed, `encode`/`decode`), and its own `using ... global` (legal — it is the defining file), importing no user code. The dependency graph is a straight line — config → generated types → generated tables → user code — so there are no cycles by construction; "references a type that doesn't exist until codegen runs" is the same relationship user code already has with tables. **`packed` changes only the Solidity-side representation — storage, columns, and events are identical either way** (N component fields, same tight-packed contiguous bytes, one range read to load, one coalesced splice to save). `packed: true` computes a UDVT width from the components (≤32 bytes; 1 stack slot, component accessors + functional `withX` updaters + `encode`/`decode` generated — whole-value load/save is a bytes-level cast since codegen owns both packing and layout order, an invariant). Without `packed`, a memory struct — and this is the _simpler_ case: members are transparent, so **no codec exists at all**; the generated file is just the struct + a load/save field handle, table code touches members directly (`spawn.x` for key encoding, named literals `Spawn({ x: ..., ... })` in the record codec), mutation is in-place, and there's no width cap beyond the 28-field ceiling — the literal answer to [#1461](https://github.com/latticexyz/mud/issues/1461). As keys, declared types expand to N columns (`position_x`, `from_x`/`to_x` — instance naming free via field names); as values, N tight-packed component fields with per-component offchain columns and a generated composite handle that is a **mini-record**: `load()`/`save()` for the whole value (whole-value `save()` coalesces components into one static splice) plus **component accessors returning ordinary primitive field handles** — `spawn().x().save(5)` splices one component without a whole-value round-trip (the deliberate partial-write pattern DUST uses on hot tables), identically for both shapes since components are real onchain fields either way. The handle model is fractal: record handle = load/save + field accessors; composite field handle = load/save + component accessors. A loaded struct mutates in place exactly like the record struct does (`s.x += 1`, the §1 precedent); packed is the value-flavored representation you opt into for stack residency and operators.
...
- `IStoreHook` removal (decided — §2 item 6; hooks are independently deployed and `registerStoreHook` is external, so removing them is a world-external break).
...
| [#3636](https://github.com/latticexyz/mud/issues/3636) pass data from before to after hooks                                                                                               | Moot — store hooks are being removed (§2 item 6).                                                                                                                                                                                                                                                                                                                                                                                                                                                                      |
